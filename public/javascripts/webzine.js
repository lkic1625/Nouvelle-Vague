...
const getPostResponse = (res) => {
	...
	$('span#viewss').html(parser.views);

	$('.postname').html(parser.title);
	$('#datetime').html(parser.createdAt);
	$('#author').html(parser.author);
	$('.content p').html(parser.description);
	$('#main-img').attr('src', parser.posterImg);
	$('.tags ul').html("");
	parser.Tags.forEach((element) => {
		$('#window-tags ul').append('<li>' + element.tag + '</li>');
	});
}

$(function() {
	
	//write-form image-uploader settings.
	$('#mainImg-preview').on('dragover', function() {
		$('#mainImg-preview').addClass('dragging')
	  }).on('dragleave', function() {
		$('#mainImg-preview').removeClass('dragging')
	  }).on('drop', function(e) {
		$('#mainImg-preview').removeClass('dragging hasImage');
		...
		if (e.originalEvent) {
		  ...
		  reader.onload = function(e) {
			...
			$('#mainImg-preview').css('background-image', 'url(' + reader.result + ')').addClass('hasImage');
	  
		  }
	  
		}
	  })
	  $('#previewImg-preview').on('click', function(e) {
		console.log('clicked')
		$('#write-previewImg').click();
	  });

	  $('#mainImg-preview').on('click', function(e) {
		console.log('clicked')
		$('#write-mainImg').click();
	  });
	 
	  $('#write-previewImg').change(function() {
		const input = this;
		if (input.files && input.files[0]) {
			const reader = new FileReader();
			
			reader.onload = function(e) {
				$('#previewImg-preview').css('background-image', `url(${e.target.result})`).addClass('hasImage');
				 
			 	$('#preview-preview').css('background-image', `url(${e.target.result})`);
			}
		...
		$("#write-mainImg").change(function() {
			const input = this;
			if (input.files && input.files[0]) {
				const reader = new FileReader();
				
				reader.onload = function(e) {
				  $('#mainImg-preview').css('background-image', `url(${e.target.result})`).addClass('hasImage');
				  $('#poster-preview').css('background-image', `url(${e.target.result})`);
				}
	  ...
	  $('#write-mainImg').change(function(e) {
...
$(function () {
	...
	$('.scene').each(function () {
		$(this).click(function () {
			//current post id
			const post_id = $(this).data('post');
			const next_post_id = post_id + 1;
			$('#next').html($('.scene[data-post=' + next_post_id + ']').html()).attr('data-post', next_post_id);
			$('#next').data('post', next_post_id);
	...
	//write form has preview-scene DOM that is preview for posts to be uploaded soon
	//that has 'scene' class but onclick event must be null.
	$('#preview-scene').off('click');
});
...
$(function () {
	...
	$('.auth-submit').click(function () {
		...
		if (formType == 'login-form') {
			...
			formData.append('posterImg', $('#write-mainImg')[0].files[0]);
	...
	//Copy contents to preview section for every keyup event
	$('#write-description').keyup(function() {
		$('#preview').html($(this).val());
	});

	//tag parsing for every keyup event
	$('#write-tags').keyup(function () {
		let string = $(this)[0].value;
		console.log($(this));
		string = string.replace(/(^|\s)(#[a-z\d-]+)/ig, "$1<li class='hashtag'>$2</li>");
		console.log(string);
		$('#preview-tags').html(string);
	});
	...
	$('.tag-nav li').each(function () {
		$(this).click(function () {
			const posts = $('.scene');
			const hashtag = $(this).html().replace('#','');
			
			//hide all posts
			posts.addClass('fix');
			posts.css('display', 'none');
			
			//select posts contain this hashtag
			//PureJS was faster than jQuery about 12% on Chrome, 21% on Firefox and 25% on Safari.
			const selectedPost = document.querySelectorAll(`[data-tag*='${hashtag}']`);
			selectedPost.forEach((post) => {
				post.style.display = 'block';
			});
			// selectedPost.style.display = 'block';
			// $('.scene[data-tag ~= ' + hashtag + ']').css('display', 'block');
		});
	});
...
function wrapWindowByMask(selector = '.window') {
	...
	// // css 스타일을 변경합니다.
	if (selector == '.article-container' || selector == '.board-container#write-box')
...
$(function () {	
	...
	$(".heart").on("click", function () {
		...
		//find post_id by next_post section
		const post_id = $('#next').data('post') - 1;
		//axios call
		axios.post(`board/posts/like/${post_id}`, {}, {
			headers: {
			...
			.then((res) => {
				if (res.status == 201) {
					...
					if (res.data.message == 'like') {
						$('.liketitle').append("<li data-post = '" + (post_id) + "' style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap; display:none'>" + $('#window-postname').html() + "</li>");
					...
					else if (res.data.message == 'dislike') {
						$(`li[data-post = ${post_id}]`)[0].remove();
					}
